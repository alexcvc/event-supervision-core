...
`.cpp` files in the library itself — everything lives under `include/app/event/*.hpp`.
...
Supporting types:
- `EventId` (`include/app/event/EventId.hpp`) — enum of all known monitored conditions; `Count` is a sizing
